...
export default function SuperuserDashboard({ setViewMode }) {
  ...
  const handleSaveProduct = async (e) => {
    ...
    try {
      ...
      // Upload any local selected images (files or Cloudinary URLs)
        ...
        for (const img of selectedLocalImages) {
          if (img.isUrl) {
            try {
              ...
              console.error("Failed to upload Cloudinary URL during creation", imageErr);
            }
          } else {
            const formData = new FormData();
            ...
            try {
              ...
              console.error("Failed to upload file during creation", imageErr);
            }
          }
        }
      }
  ...
  const handleAddCloudinaryUrl = async () => {
    if (!cloudinaryUrl.trim()) return;
    const url = cloudinaryUrl.trim();
    
    if (editingProduct) {
      try {
        const res = await api.post('/products/images/', {
      ...
      setSelectedLocalImages(prev => [
        ...prev,
        {
          isUrl: true,
          url,
          isMain: selectedLocalImages.length === 0
        }
      ]);
      setCloudinaryUrl('');
    }
                      ...
                      <tr key={order.id}>
                        ...
                        <td>
                          <span className={`${s.statusPill} ${
                            order.order_status === 'Delivered' ? s.statusDelivered :
              ...
              <form onSubmit={handleSaveProduct}>
                <div className={s.modalBody}>
                  ...
                  <div className={s.imagesSection}>
                    <h4 className={s.formLabel} style={{ marginBottom: '0.75rem' }}>Product Photos & Cloudinary URLs</h4>
                    
                    {/* Cloudinary URL Input Slot */}
                    ...
                    <div className={s.imagesGrid}>
                      ...
                      {selectedLocalImages.map((img, idx) => (
                        <div className={s.imgWrapper} key={idx}>
                          <img src={img.isUrl ? img.url : img.previewUrl} alt="" className={s.gridImg} />
                      ...
                      <label className={s.uploadWrapper}>
                        <CameraIcon />
                        <span className={s.uploadText}>Local Files</span>
